(()=>{
  ...
  const TYPE=String(C.institutionType||"basic_jhs");
  // Only Senior High School needs this extension. Basic/JHS uses the core
  // academic workspace. All other institution models are retired/unsupported.
  if(TYPE!=="senior_high")return;
  ...
  const client=()=>{
    if(window.EdusentiaFinanceRuntime?.S?.client)return window.EdusentiaFinanceRuntime.S.client;
    if(standaloneClient)return standaloneClient;
    if(!window.supabase?.createClient||!C.supabaseUrl||!C.supabaseAnonKey)return null;
    standaloneClient=window.supabase.createClient(C.supabaseUrl,C.supabaseAnonKey,{auth:{persistSession:true,autoRefreshToken:false,detectSessionInUrl:false}});
  ...
  async function rows(table,select="*",builder=null){const c=client();if(!c)throw new Error("Academic service unavailable");let q=c.from(table).select(select);if(builder)q=builder(q);const {data,error}=await q;if(error)throw error;return data||[]}
  async function insert(table,payload){const {data,error}=await client().from(table).insert(payload).select().single();if(error)throw error;return data}
  async function remove(table,id){const {error}=await client().from(table).delete().eq("id",id);if(error)throw error}
  function notify(text,kind="info"){let root=byId("institutionAcademicsToast");if(!root){root=document.createElement("div");root.id="institutionAcademicsToast";root.className="toast-stack";document.body.append(root)}const el=document.createElement("div");el.className=`toast ${kind}`;el.textContent=text;root.append(el);setTimeout(()=>el.remove(),4500)}
  ...
  async function renderSHS(){
    const [programmes,levels,subjects,years,students,enrollments,mappings]=await Promise.all([
      rows("academic_programmes","*",q=>q.eq("institution_scope","senior_high").order("name")),
      rows("academic_levels","*",q=>q.eq("institution_scope","senior_high").order("level_order")),
      rows("subjects","id,name,code,active",q=>q.eq("active",true).order("display_order")),
      rows("academic_years","id,name,start_date,end_date",q=>q.order("start_date",{ascending:false})),
      rows("students","id,admission_no,first_name,middle_name,last_name,status",q=>q.is("deleted_at",null).order("last_name")),
      rows("student_programme_enrollments","*",q=>q.order("created_at",{ascending:false})),
      rows("shs_programme_subjects","*",q=>q.order("display_order"))
    ]);
    const programmeIds=new Set(programmes.map(x=>x.id));
    const shsEnrollments=enrollments.filter(x=>programmeIds.has(x.programme_id));
    ...
    forms[0]?.addEventListener("submit",ev=>{ev.preventDefault();const f=ev.currentTarget;run(()=>insert("academic_programmes",{code:val(f,"code"),name:val(f,"name"),institution_scope:"senior_high",award_type:val(f,"award_type"),duration_years:num(f,"duration_years")}),"Programme added")});
    forms[1]?.addEventListener("submit",ev=>{ev.preventDefault();const f=ev.currentTarget;run(()=>insert("academic_levels",{programme_id:val(f,"programme_id"),code:val(f,"code"),name:val(f,"name"),institution_scope:"senior_high",level_order:num(f,"level_order")}),"Level added")});
    forms[2]?.addEventListener("submit",ev=>{ev.preventDefault();const f=ev.currentTarget;run(()=>insert("shs_programme_subjects",{programme_id:val(f,"programme_id"),level_id:val(f,"level_id")||null,subject_id:val(f,"subject_id"),subject_category:val(f,"subject_category"),required:checked(f,"required")}),"Subject assigned")});
    forms[3]?.addEventListener("submit",ev=>{ev.preventDefault();const f=ev.currentTarget;run(()=>insert("student_programme_enrollments",{student_id:val(f,"student_id"),programme_id:val(f,"programme_id"),academic_year_id:val(f,"academic_year_id"),level_id:val(f,"level_id")||null,status:val(f,"status")}),"Student programme assigned")});
    root.querySelectorAll("[data-delete-programme]").forEach(b=>b.onclick=()=>run(()=>remove("academic_programmes",b.dataset.deleteProgramme),"Programme removed"));
    root.querySelectorAll("[data-delete-level]").forEach(b=>b.onclick=()=>run(()=>remove("academic_levels",b.dataset.deleteLevel),"Level removed"));
    root.querySelectorAll("[data-delete-mapping]").forEach(b=>b.onclick=()=>run(()=>remove("shs_programme_subjects",b.dataset.deleteMapping),"Subject mapping removed"));
    root.querySelectorAll("[data-delete-enrollment]").forEach(b=>b.onclick=()=>run(()=>remove("student_programme_enrollments",b.dataset.deleteEnrollment),"Student assignment removed"));
  }

  async function renderActive(){
    if(!academicPage()||activeTab!=="shs-programmes")return;
