...
plan_tiering_count="$(psql "$TEMPLATE_URL" -Atc "select count(*) from platform.license_plans p where
...
user_workspace_parity_ok="$(psql "$TEMPLATE_URL" -Atc "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='students' and column_name='profile_id') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name='must_change_password') and to_regclass('public.students_profile_id_uidx') is not null and has_function_privilege('edusentia_worker_runtime','public.list_profiles_with_access()','execute') and has_function_privilege('edusentia_worker_runtime','public.admin_validate_user_bundle(uuid,jsonb,boolean)','execute') and has_function_privilege('edusentia_worker_runtime','public.admin_apply_user_bundle(uuid,jsonb)','execute') and has_function_privilege('edusentia_worker_runtime','public.neon_identity_create_auth_user(uuid,uuid,uuid,text,text,text,boolean,text,boolean,boolean,text,text,text)','execute') and has_function_privilege('edusentia_worker_runtime','public.neon_identity_update_auth_user(uuid,uuid,uuid,text,text,text,boolean,text,boolean,boolean)','execute') and has_function_privilege('edusentia_worker_runtime','public.neon_identity_link_guardian(uuid,uuid,uuid)','execute') and has_function_privilege('edusentia_worker_runtime','public.neon_identity_reset_password(uuid,uuid,uuid,text,text,text,boolean)','execute') and has_function_privilege('edusentia_worker_runtime','public.neon_identity_delete_auth_user(uuid,uuid,uuid,text)','execute') and exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid join pg_namespace n on n.oid=t.typnamespace where n.nspname='public' and t.typname='app_role' and e.enumlabel='accountant') and exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid join pg_namespace n on n.oid=t.typnamespace where n.nspname='public' and t.typname='app_role' and e.enumlabel='student') and position('on conflict on constraint tenant_memberships_pkey' in lower(pg_get_functiondef('public.neon_identity_create_auth_user(uuid,uuid,uuid,text,text,text,boolean,text,boolean,boolean,text,text,text)'::regprocedure)))>0 and position('''teacher_records''' in pg_get_functiondef('public.list_profiles_with_access()'::regprocedure))>0 and position('''headteacher_records''' in pg_get_functiondef('public.list_profiles_with_access()'::regprocedure))>0 and position('''accountant_records''' in pg_get_functiondef('public.list_profiles_with_access()'::regprocedure))>0 and position('''student_records''' in pg_get_functiondef('public.list_profiles_with_access()'::regprocedure))>0 and position('authn.users' in pg_get_functiondef('public.list_profiles_with_access()'::regprocedure))>0 and exists(select 1 from pg_policies where schemaname='public' and tablename='teachers' and policyname='neon_certified_runtime_owner' and 'edusentia_runtime'=any(roles)) and exists(select 1 from pg_policies where schemaname='public' and tablename='headteachers' and policyname='neon_certified_runtime_owner' and 'edusentia_runtime'=any(roles))")"

backup_schedule_ok="$(psql "$TEMPLATE_URL" -Atc "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='school_settings' and column_name='backup_schedule_mode') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='backup_exports' and column_name='trigger_mode') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='backup_exports' and column_name='heartbeat_at') and has_function_privilege('edusentia_worker_runtime','public.backup_worker_schedule_policy()','execute') and has_function_privilege('edusentia_worker_runtime','public.backup_worker_set_schedule_policy(text,uuid)','execute') and has_function_privilege('edusentia_worker_runtime','public.backup_worker_read_batch(uuid,jsonb)','execute')")"
grading_scale_contract_ok="$(psql "$TEMPLATE_URL" -Atc "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='grading_scales' and column_name='interpretation' and is_nullable='NO') and position('interpretation' in pg_get_functiondef('public.save_grading_scale(jsonb)'::regprocedure))>0")"

echo "Tenant template validation: runtime_schema=$tenant_runtime_schema compat_schema=$compat_schema compat_version=$compat_version bootstrap_count=$bootstrap_count worker_rpc_count=$worker_rpc_count plan_tiering_count=$plan_tiering_count runtime_acl_ok=$runtime_acl_ok admission_reuse_ok=$admission_reuse_ok student_photo_contract_ok=$student_photo_contract_ok principal_photo_contract_ok=$principal_photo_contract_ok operational_repairs_ok=$operational_repairs_ok user_workspace_parity_ok=$user_workspace_parity_ok backup_schedule_ok=$backup_schedule_ok grading_scale_contract_ok=$grading_scale_contract_ok"
