...
backup_schedule_ok="$(psql "$TEMPLATE_URL" -Atc "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='school_settings' and column_name='backup_schedule_mode') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='backup_exports' and column_name='trigger_mode') and has_function_privilege('edusentia_worker_runtime','public.backup_worker_schedule_policy()','execute') and has_function_privilege('edusentia_worker_runtime','public.backup_worker_set_schedule_policy(text,uuid)','execute')")"
