...
declare v_plan_id uuid; v_user_id uuid; v_certified_settings_id uuid;
begin
  if p_tenant_id is null then raise exception 'tenant_id_required' using errcode='22023'; end if;
  if p_tenant_code !~ '^[A-Z]{3}-[0-9]{6}
  ...
  -- Keep the certified compatibility settings in lockstep with the Neon-native
  -- school record. The stable UI/RPC surface reads public.school_settings.
  select id into v_certified_settings_id
  from public.school_settings
  order by created_at,id
  limit 1
  for update;

  if v_certified_settings_id is null then
    insert into public.school_settings(
      school_name,email,tenant_code,identifier_root,institution_type,updated_at
    )
    values(
      trim(p_school_name),
    ...
    returning id into v_certified_settings_id;
  else
    ...
    set school_name=trim(p_school_name),
        ...
        updated_at=now()
    where id=v_certified_settings_id;
  end if;
...
commit;
 then raise exception 'invalid_tenant_code' using errcode='22023'; end if;
  if p_institution_type not in ('basic_jhs','senior_high') then raise exception 'unsupported_institution_type' using errcode='22023'; end if;
  if p_expires_at<=p_starts_at then raise exception 'invalid_licence_window' using errcode='22023'; end if;
  select id into v_plan_id from platform.license_plans where code=lower(trim(p_plan_code)) and active;
  if v_plan_id is null then raise exception 'plan_not_found' using errcode='P0002'; end if;
