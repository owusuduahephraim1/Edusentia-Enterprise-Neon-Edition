...
begin
  if not public.is_system_admin() then raise exception 'Access denied' using errcode='42501'; end if;
  if v_tenant_id is null then raise exception 'Authenticated tenant context is required' using errcode='42501'; end if;
  if target_range_key not in ('early_years','basic_1_6','basic_7_9') then raise exception 'Invalid report-card class range' using errcode='22023'; end if;
  expected_prefix:='tenants/'||v_tenant_id::text||'/report-card-templates/'||target_range_key||'/';
  if clean_path='' or clean_path not like (expected_prefix||'%') or clean_path like '%..%' then raise exception 'Invalid template storage path' using errcode='22023'; end if;
  if coalesce(btrim(target_original_name),'')='' or length(target_original_name)>255 then raise exception 'Invalid template file name' using errcode='22023'; end if;
  if target_mime_type not in ('application/pdf','application/vnd.openxmlformats-officedocument.wordprocessingml.document') then raise exception 'Unsupported template file type' using errcode='22023'; end if;
  if coalesce(target_file_size,0)<=0 or target_file_size>20971520 then raise exception 'Template file must be between 1 byte and 20 MB' using errcode='22023'; end if;
  if not exists(
  ...
  ) then raise exception 'The uploaded report-card template is not available in protected storage' using errcode='22023'; end if;
  if coalesce(length(target_checksum),0)>128 then raise exception 'Invalid template checksum' using errcode='22023'; end if;
