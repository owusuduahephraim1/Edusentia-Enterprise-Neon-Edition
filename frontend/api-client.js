(() => {
  ...
  async function downloadPrincipalPhoto(principalId,photoPath){
    const response=await fetch(`${apiBase}/api/files/principal-photo?id=${encodeURIComponent(String(principalId||""))}&path=${encodeURIComponent(String(photoPath||""))}`,{credentials:"include",headers:{accept:"image/avif,image/webp,image/png,image/jpeg,image/*"}});
    if(!response.ok){const payload=await response.json().catch(()=>({}));const e=payload?.error||{};throw new ApiError(e.message||`Photograph download failed (${response.status})`,response.status,e.code||"photo_download_failed",e.details);}
    return response.blob();
  }
  ...
  window.EdusentiaApi=Object.freeze({
    ...
    shsAcademicConsole:async()=>((await request("/api/academics/shs"))?.result||{}),
