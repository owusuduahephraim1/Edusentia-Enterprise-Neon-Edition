(() => {
  ...
  async function useRecoveryCode(){const code=prompt("Enter one Edusentia MFA recovery code (EDS-XXXXX-XXXXX):");if(code===null)return;try{await edge("redeem_recovery_code",{recovery_code:String(code).trim()});alert("The lost authenticator was revoked. Edusentia will reload and require a new authenticator QR enrollment.");location.reload()}catch(error){alert(String(error?.message||error).replaceAll("_"," "))}}
...
(() => {
  ...
  async function loadFinance(){if(document.documentElement.dataset.edusentiaFinanceLoaded==="1")return;for(let i=0;i<240&&!tenantReady();i++)await new Promise(r=>setTimeout(r,250));if(!tenantReady())return;document.documentElement.dataset.edusentiaFinanceLoaded="1";loadCss();try{await loadScript("finance-core.js","core");installFinancePrintFrame();await loadScript("finance-payroll.js","payroll");await loadScript("finance-accounts.js","accounts");await loadScript("finance-professional-receipts.js","receipts");await loadScript("finance-portals.js","portals");installFinanceEntitlementUi();installFeeGroupUi()}catch(error){console.error("edusentia_finance_modules_load_failed",error);document.documentElement.dataset.edusentiaFinanceLoaded="0"}}
