(() => {
  ...
  async function renderOperations(){
    ...
    try{
      ...
      byId("generateMissingReports").onclick=async()=>{const classId=byId("operationsClass").value;if(!classId){toast("Select a class","Choose the class before previewing missing reports.","warning");return;}try{const preview=await certified("bulk_generate_missing_reports",{target_term_id:termId,target_class_id:classId,preview_only:true});if(!num(preview?.missing_reports)){toast("No missing reports","Every active student already has a report for this term.");return;}if(!confirm(num(preview.missing_reports)+" missing report record(s) will be created. Existing reports will not be changed. Continue?"))return;const result=await certified("bulk_generate_missing_reports",{target_term_id:termId,target_class_id:classId,preview_only:false});toast("Draft reports generated",num(result?.created_reports)+" report record(s) created.");await renderOperations();}catch(e){toast("Reports not generated",friendly(e),"error");}};
      byId("recoveryRun")?.addEventListener("click",async()=>{if(!latestBackup?.id||!confirm("Run a non-destructive recovery rehearsal using the latest completed backup?"))return;const b=byId("recoveryRun");b.disabled=true;try{const data=await api().scheduledBackup("recovery_test",{backup_id:latestBackup.id});toast("Recovery rehearsal passed",num(data?.checked_tables)+" tables, "+num(data?.checked_rows)+" rows, and "+num(data?.checked_storage_objects)+" storage objects verified.");await renderOperations();}catch(e){toast("Recovery rehearsal failed",friendly(e),"error");}finally{b.disabled=false;}});
      byId("content").querySelectorAll("[data-correction-review]").forEach(b=>b.onclick=async()=>{const note=prompt((b.dataset.decision==="approved"?"Approval conditions":"Rejection reason"))||"";try{await certified("review_report_correction",{target_request_id:b.dataset.correctionReview,decision:b.dataset.decision,review_note_text:note||null});toast("Correction request reviewed");await renderOperations();}catch(e){toast("Review not saved",friendly(e),"error");}});
